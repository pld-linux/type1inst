...
%description
type1inst is a utility which scans Type 1 PostScript font files 
and generates automatically the fonts.scale file which the X  
Window System  uses to pick up fonts in the current directory 
and/or the Fontmap file which Ghostscript uses to map file  
names to  font names. It can also generate simple font sample 
sheets which show what your fonts look like.

type1inst is a Perl script so you will need to have that  installed 
on your system. The pfbtops utility (which comes as part of GNU 
groff) is also required.
...
%files
%attr(644, root, root, 755) %doc README type1inst-*.lsm
%attr(755, root, root) /usr/bin/*
%attr(644, root,  man) /usr/man/man1/type1inst.1
